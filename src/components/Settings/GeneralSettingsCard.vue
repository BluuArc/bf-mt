<template>
  ...
  <!-- <v-card raised>
    ...
    <v-card-text>
      <v-switch v-model="lightMode" label="Use Light Theme"/>
    </v-card-text>
    <v-card-text>
      <h3 class="subheading">Active Server</h3>
      <v-radio-group v-model="activeServer" row>
        <v-radio
          v-for="server in servers"
          :key="server"
          :label="server.toUpperCase()"
    ...
    <v-card-actions>
      <v-btn :disabled="!formHasChanged" flat @click="formSubmit">Apply Changes</v-btn>
      <v-btn flat @click="formReset">Cancel</v-btn>
    </v-card-actions>
  </v-card> -->
</template>
...
export default {
  props: {
    logger: {
      default: () => new Logger({ prefix: '[GENERAL-SETTINGS]' }),
    },
  },
  components: {
    GenericSettingsCard,
