  ...
  <v-container grid-list-sm class="pb-5">
            ...
            <v-container fluid class="pa-0">
              <v-layout row>
                ...
                <v-flex xs4 class="center-align-parent text-xs-center">
                  <span class="center-align-container">Showing {{ allSortedEntries.length }}<br>Leader Skills</span>
                </v-flex>
              </v-layout>
              <v-layout row>
              </v-layout>
    ...
    <v-layout row>
      <v-dialog v-model="showDialog" fullscreen hide-overlay transition="dialog-bottom-transition">
        <v-card>
          <v-toolbar fixed>
            <v-btn icon to="/multidex/leader-skills">
              <v-icon>close</v-icon>
            </v-btn>
            <v-toolbar-title>
              <span style="margin-top: auto; margin-bottom: auto;" class="pl-2">
                <span v-if="pageDb[leaderId]">
                  {{ pageDb[leaderId].name }}
                </span>
                <span v-else-if="leaderId">
                  (ID: {{ leaderId }})
                </span>
              </span>
            </v-toolbar-title>
          </v-toolbar>
          <v-card-text v-if="leaderId" class="pl-0 pr-0 pt-5">
            <skill-info :leaderId="leaderId"/>
          </v-card-text>
        </v-card>
      </v-dialog>
    </v-layout>
...
export default {
  ...
  watch: {
    ...
    leaderId (newValue) {
      ...
      if (this.pageDb.hasOwnProperty(newValue)) {
        document.title = `BF-MT - Leader Skills - ${this.pageDb[newValue].name}`;
      }
    },
    filterOptions: {
      deep: true,
  ...
  mounted () {
    this.resetFilters();
    if (this.leaderId && !this.isLoading) {
      this.showDialog = true;
    }
  },
  methods: {
